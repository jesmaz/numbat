...
class Equal : public Instruction {
	...
	public:
		...
		const nir::Instruction * recreate (const std::vector <Argument> & replacmentArgs) const {assert (replacmentArgs.size () == 2); return new Equal (getType (), replacmentArgs [0], replacmentArgs [1], getIden ());}
