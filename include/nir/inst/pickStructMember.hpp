...
class PickStructMember : public Instruction {
	...
	private:
		...
		PickStructMember (const Type * type, Argument parent, size_t index, const string & memIden, symbol iden) : Instruction ({type}, {iden}), parent (parent), index (index), memIden (memIden) {}
