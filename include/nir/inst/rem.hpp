...
class Rem : public Instruction {
	...
	public:
		...
		const nir::Instruction * recreate (const BasicArray <Argument> & replacmentArgs) const {assert (replacmentArgs.size () == 2); return new Rem (getType (), replacmentArgs [0], replacmentArgs [1], getIden ());}
