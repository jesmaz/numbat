...
Function::Function (symbol label, LINKAGE linkage) : entry (new Block (label)), blocks ({entry}), label (label), linkage (linkage) {}

Function::Function (const BasicArray <const Parameter *> & args, const BasicArray <const Parameter *> & ret, symbol label, LINKAGE linkage) : entry (new Block (label)), blocks ({entry}), label (label), args (args), ret (ret), retTypes (getInstType (ret)), type (FunctionType::get (getInstType (args), retTypes)), linkage (linkage) {}
